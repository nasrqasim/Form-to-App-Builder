...
import { Save, Loader2, Rocket, ArrowLeft, Trash2 } from 'lucide-react';
import ProjectNavbar from '@/components/ProjectNavbar';
import ImageUpload from '@/components/ImageUpload';

export default function EditPage({ params: paramsPromise }) {
    const params = use(paramsPromise);
    const { appName, id } = params;
    ...
    const fetchData = async () => {
        try {
            const res = await fetch(`/api/apps/${appName}/update/${id}`);
            const data = await res.json();
            if (res.ok) {
                setAppMeta(data.appMeta);
                const initial = { ...data.item };
                data.appMeta.fields.forEach(f => {
                    if (f.fieldType === 'date' && initial[f.fieldName]) {
                        initial[f.fieldName] = new Date(initial[f.fieldName]).toISOString().split('T')[0];
                    }
                });
                setFormData(initial);
            } else {
    ...
    if (!appMeta || !formData) return null;
        ...
        <div className="min-h-screen bg-gray-50 dark:bg-gray-900 transition-colors duration-300">
            ...
            <main className="max-w-3xl mx-auto py-12 px-4">
                <div className="mb-10 text-center">
                    <h1 className="text-4xl font-black text-gray-900 dark:text-white tracking-tight">Edit Entry</h1>
                    <p className="text-gray-500 dark:text-gray-400 mt-2">Update information for this {appMeta.name.replace(/s$/i, '')} record</p>
                </div>
